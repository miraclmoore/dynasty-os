...
- [ ] **QOL-01**: User receives a toast notification confirming every successful write operation (game log, player edit, stat entry)
- [ ] **QOL-02**: User can undo the last single destructive action (delete or edit) for games, players, and season stats
- [ ] **QOL-03**: All list and table filter selections persist across navigation within the same app session
- [ ] **QOL-04**: User can open a command palette (Ctrl+K / Cmd+K) to navigate to any page or trigger quick actions from any screen
...
- [ ] **COMM-01**: User can track coaching staff lifecycle — hire, fire, promote roles with tenure dates and scheme notes
...
- [ ] **COMM-04**: CFB users can log NIL deals per player (amount, brand, duration) and view total spend by class and position
...
Which phases cover which v2.0 requirements. Updated during roadmap creation.
