...
## Current Milestone: v2.2 Handoff Overhaul

**Goal:** Harden the app with safety/security fixes identified in a comprehensive audit and ship the highest-impact new features — completing the screenshot-to-DB pipeline, upgrading Madden sync with real stat lines, wiring the AI queue, adding CFB recruiting tools, and improving data entry UX throughout.

**Target features:**
- Top-level error boundary + undo type safety + dead dependency removal + N+1 query fix (Safety)
- All Anthropic API calls routed through Tauri command + API key moved to plugin-store + localStorage fully migrated (Security)
- Rivalry key moments in Dexie, bowl opponent + key events on Season, dev trait + deal breaker + redshirt on Player, recruit motivation fields (Data Model)
- Player stats screenshot → fuzzy match → save to DB; recruiting-motivations screen type; multi-image import (Screenshot Pipeline)
- Madden sync extracts PlayerStats; auto-detects save file path (Madden Sync)
- Hard Sell calculator; transfer risk tags; recruit → roster conversion; draft pick auto-updates player status (Recruiting Tools)
- AI queue processor wired; auto-blurb generation made explicit; Haiku/Sonnet model routing; game narrative trigger (AI Queue)
- Quick Score dashboard widget; CSV game + roster import; screenshot tips panel; video frame extraction (Data Entry UX)
- CommandPalette covers all 24 pages; React Router replaces custom SPA router (Navigation)
- Dev trait in trade calculator; filter store persist; component size refactor; TickerBar sport toggle; game version registry; final verification (Polish)

## Previous Milestone: v2.1 UX/UI Polish
...
**Status:** Phases 14 complete. Phases 15–18 pending (deferred to allow handoff overhaul to proceed first).

## Milestone Before That: v2.0 The Living Dynasty
...
- [ ] Recruiting tools: Hard Sell calculator; transfer risk tags; recruit → roster; draft pick → player status
- [ ] AI queue: wire processor; explicit blurb generation; Haiku/Sonnet routing; game narrative trigger
- [ ] Data entry UX: Quick Score widget; CSV game + roster import; screenshot tips; video import
- [ ] Navigation: CommandPalette all 24 pages; React Router migration
- [ ] Polish: dev trait in trade calc; filter store persist; component refactor; TickerBar toggle; game version registry; key events on timeline
