...
<task type="auto">
  ...
  <action>
    ...
    **PlayerProfilePage.tsx** — Wire Legacy Card auto-generation on departure:
    - After the departure form submits and player status changes:
      ...
      3. Store the blurb in localStorage key `legacy-blurb-{playerId}`. Do NOT use the player's notes field (notes is for user bio content and would be clobbered). Note: blurbs stored in localStorage are not included in JSON export — this is an acceptable V1 limitation.
    ...
    **LegendsPage.tsx** — Program Legends gallery:
    ...
    - Load all playerSeasons for the dynasty in a single call via getPlayerSeasonsByDynasty(dynastyId) from player-season-service, then partition by playerId in memory before building LegacyCardData per player. Do NOT loop and call getPlayerSeasonsByPlayer per departed player.
    - For each departed player, build LegacyCardData using the partitioned seasons
  ...
  <done>
    Legacy Cards auto-generate at player departure with career stats and awards. AI blurb generates via Claude Haiku when API key is present, degrades gracefully when absent. Blurb stored in localStorage key `legacy-blurb-{playerId}` (not in player notes field; not included in JSON export — acceptable V1 limitation). PNG export uses Tauri dialog + fs (not blob URLs). Legends gallery loads all playerSeasons in a single bulk query and partitions by playerId in memory (no N+1). Legends gallery shows all departed player cards with position/era/award filters.
  </done>
</task>
